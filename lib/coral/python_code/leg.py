...
def draw_pose1(dwg, pose, src_size, inference_box, color='yellow', threshold=0.2):
    ...
    for a, b in EDGES:
        ...
        print(EDGES[a], xys[a], EDGES[b], xys[b])


def draw_pose2(dwg, pose, src_size, inference_box, color='yellow', threshold=0.2):
    # start_server = websockets.serve(accept, "192.168.0.13", 9998)
    box_x, box_y, box_w, box_h = inference_box
    scale_x, scale_y = src_size[0] / box_w, src_size[1] / box_h
    xys = {}
    data_list = []
    ...
    for a, b in EDGES:
        ...
        bx, by = xys[b]
        dwg.add(dwg.line(start=(ax, ay), end=(bx, by), stroke=color, stroke_width=2))
        #data = EDGES[a] + xys[a] + EDGES[b] + xys[b]
        data = {a : xys[a], b : xys[b]}
...
async def accept(ws, path):
    ...
    while True:
        ...
        with open(file_path, 'w') as outfile:
            dataFromCoral = json.dump(data_list, outfile, indent=4)
        ...
        for line in json_data:
            for item in line:
                text = text + "keypointType." + str(item) + " " + str(line[item][0]) + " " +  str(line[item][1]) + "end"
                # print(item)   # item은 KeypointType 숫자 나옴 2개씩 한쌍
                # print(line[item])  # line[item] x, y 좌표 나옴 2개씩 한쌍
...
def webSocket():
    ws = websockets.serve(accept, "192.168.0.12", 9998)
...
def main(q):
    ...
    def render_overlay(engine, output, src_size, inference_box):
        ...
        for pose in outputs:
            # draw_pose1(svg_canvas, pose, src_size, inference_box)
            draw_pose2(svg_canvas, pose, src_size, inference_box) #arguments must be 2-4
...
if __name__ == '__main__':
    ...
    t1.start()
    t2 = threading.Thread(target=asyncio.run(webSocket(), name = "[Daemon]", args= (q,)))
    t2.setDaemon(True)

    t2.start()
    # print("t1 start")

    # loop.call_soon_threadsafe(ws.shutdown)
    t2.join()
    #t2.join()
